...
## 📌 Sobre o Projeto
O **POCAMARKET** é uma aplicação web (Single Page Application) desenvolvida para otimizar e automatizar o funil de vendas de photocards de K-Pop. A ferramenta atua como uma interface de conversão: ela educa o cliente através de regras de compra e, em seguida, oferece uma calculadora automatizada que converte valores em dólar para real, já embutindo taxas operacionais e de comissão.
...
* **Design Focado em Conversão:** Estrutura dividida em duas etapas (Educação -> Ação). O botão final pulsa (CSS Animation) atraindo o clique no momento de maior interesse do usuário.
* **Estética Premium (Glassmorphism):** Interface construída com o efeito de "vidro fosco", utilizando desfoque de fundo e bordas suaves, alinhada com a paleta de cores *Rose Quartz* para gerar identificação com o público-alvo (fandom de K-Pop).
